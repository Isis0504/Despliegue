...
async function renderAdmin(contenedor) {
  ...
  contenedor.innerHTML = `
        <section style="max-width:100%; margin:0 auto;">
            <h2 class="tituloModulo">Administración de Usuarios</h2>
                ...
                <tbody>
                    ${usuarios.map(u => `
                        <tr data-id="${u.id}">
                            ...
                            <td>
                                <span class="status-tag ${u.estado === 'aprobado' ? 'status-aprobado' : 'status-pendiente'}">
                                    ${escapeHtml(u.estado || "pendiente")}
                                </span>
                            </td>
                            <td>${new Date(u.creado_en).toLocaleDateString()}</td>
                            <td>
                                ${u.estado === "pendiente" ? `<button class="btnPrimario btn-aprobar btn-small" data-id="${u.id}">Aprobar</button>` : ""}
                                <button class="btnSecundario btn-editar btn-small" data-id="${u.id}">Editar</button>
                                <button class="logoutBtn btn-eliminar btn-small" data-id="${u.id}">Eliminar</button>
                            </td>
                        </tr>
                    `).join("")}
                </tbody>
...
/* =========================
   ...
   ========================= */
// modules/perfil.js - Vista RESIDENTE / COMITÉ
// ...
async function renderResidenteComite(contenedor, perfil) {
    contenedor.innerHTML = `
        <section class="formContainer" style="max-width:520px; margin:20px auto;">
            <h2 class="tituloModulo">Mi Perfil</h2>
            <form id="form-mi-perfil" class="form-perfil-display">
                
                <div class="input-group">
                    <label>Nombre</label>
                    <input value="${escapeHtml(perfil.nombre)}" disabled>
                </div>

                <div class="input-group">
                    <label>Correo</label>
                    <input value="${escapeHtml(perfil.correo || "")}" disabled>
                </div>
...
                </div>
                ...
                <div class="input-group">
                    <label>Estado</label>
                    <input class="${perfil.estado === 'aprobado' ? 'status-aprobado' : 'status-pendiente'} status-tag" 
                           value="${escapeHtml(perfil.estado || "pendiente")}" disabled>
                </div>

                <div class="form-actions" style="margin-top:20px;">
                    <button type="submit" class="btnPrimario">ACTUALIZAR TELÉFONO</button>
                </div>
            </form>
        </section>
    `;
    // ... (Listener se mantiene igual) ...
